...
int send_recv_command(ipmi_cmd_t *msg) {
    if (DEBUG_LOG >= 2) {
        /* common print */
    ...
    /* IPMI-RAW string - [LUN] [NETFN] [COMMAND] */
    char ipmi_common[3][5];
    sprintf(ipmi_common[0], "0x%x", msg->netfn & 0x03);
    sprintf(ipmi_common[1], "0x%x", msg->cmd);
    sprintf(ipmi_common[2], "0x%x", msg->netfn >> 2);
    ...
    /* IPMI-RAW string - [DATA] */
    char ipmi_data[msg->data_len][5];
    int init_idx = 0;
    if (oem_flag) {
        sprintf(ipmi_data[0], "0x%x", IANA_1);
        sprintf(ipmi_data[1], "0x%x", IANA_2);
        sprintf(ipmi_data[2], "0x%x", IANA_3);
        init_idx += 3;
    }
    for (int i=init_idx; i<msg->data_len; i++) {
        sprintf(ipmi_data[i], "0x%x", msg->data[i-init_idx]);
    }

    /* TODO: Make command string, then send out command */
/*
    char *img_tx_buf[5*(sizeof(ipmi_common) + sizeof(ipmi_data))];
    // append prefix
    asprintf(&img_tx_buf[0], PREFIX_IPMI_RAW);
    // append common part
    for (int i=1; i<(sizeof(ipmi_common)/5); i++) {
        asprintf(&img_tx_buf[i], ipmi_common[i-1]);
    }
    // append data part
    for (int i=1+(sizeof(ipmi_common)/5); i<(sizeof(ipmi_data)/5); i++) {
        asprintf(&img_tx_buf[i], ipmi_data[i-1-(sizeof(ipmi_common)/5)]);
    }

    // send ipmi-raw command out
    if (ipmi_raw_main( (sizeof(ipmi_common)+sizeof(ipmi_data))/5 + 1, img_tx_buf)) {
        return 1;
    }
*/
...
int do_bic_update(uint8_t *buff, uint32_t buff_len) {
    ...
    while(cur_msg_offset < buff_len) {
        ...
        //memcpy(cmd_data.length, &msg_len, sizeof(cmd_data.length));
